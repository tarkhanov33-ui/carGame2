...
class GameRender(
    ...
    public fun renderAll(engine: GameEngine) {
    ...
    public fun renderScore(score: Int)  {
        scoreTv.text = "Score:$score"
    }
    public fun renderCar(lane: Int) {
        for (i in cars.indices) {
            cars[i].visibility = if (i == lane) View.VISIBLE else View.INVISIBLE
        }
    }
    public fun renderMatrix(viewGrid: Array<Array<ImageView>>, matrix: Array<IntArray>) {
    ...
    public fun renderLives(lives: Int) {
